...
Usage:
    dataset = ChunkedTokenDataset(
        chunk_dir="data/embeddings/token_full",
        text_emb_path="data/embeddings/token_full/coco_train_txt.pt",
        batch_size=128,
        seed=42,
    )
    for epoch in range(20):
        for img_tokens, txt_embs in dataset.epoch_iterator(epoch, device):
            # img_tokens: (B, 257, 768) float32 on GPU
            # txt_embs: (B, 768) float32 on GPU
            ...
"""
...
class ChunkedTokenDataset:
    ...
    def __init__(
        ...
        # Load metadata
        meta_path = self.chunk_dir / "coco_train_chunk_metadata.json"
        ...
        # Load text embeddings (small: 118K × 768 float32 = ~347MB)
        ...
        logger.info(
            "ChunkedTokenDataset: %s split, %d samples across %d chunks, bs=%d",
            split, self._n_samples, self.num_chunks, batch_size,
        )
    ...
    def epoch_iterator(
        ...
        """Yield (img_tokens, txt_embs) batches for one epoch.
